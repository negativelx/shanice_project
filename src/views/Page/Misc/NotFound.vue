<template>
    ...
    <div class="exception-body notfound relative" style="min-height: 80vh;">
        <div class="exception-panel">
            <h1>404</h1>
            <h3>{{ t('not found') }}</h3>
            <div class="text-3xl md:text-4xl font-bold mt-5 mb-3">{{ t('Page not found') }}</div>
            <div class="text-xl md:text-2xl font-normal">{{
                    t('The page that you are looking for does not exist')
                }}
            </div>
        </div>
        <div class="absolute" style="bottom: 10vh; left:0; right:0;">
            <LangSwitch/>
        </div>
    </div>
</template>
